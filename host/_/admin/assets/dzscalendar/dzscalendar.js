...
//                $(document).undelegate('.hasEvent', 'click');
                ...
                $(document).delegate('.dzscalendar .curr-months-date.hasEvent', 'click', click_event);
                $(document).delegate('.dzscalendar .curr-months-date.hasEventForHover', 'click', click_event);
				...
				if(o.mode == 'blogevents' && o.mode_blogevents_clickNonEventTriggersDefaultContent=='on'){

                    $(document).undelegate('.dzscalendar.mode-blogevents .mon-body .week-day.curr-months-date', 'click');
                    $(document).delegate('.dzscalendar.mode-blogevents .mon-body .week-day.curr-months-date', 'click', click_event);
                }
...
//                clearTimeout(inter_hidetooltips);
                ...
                if(o.mode=='blogevents'){
                   // console.info(o.mode, cont, _t);
					...
					$('.add_new_event').show();
					$('.event_date_show').show();
					$('.event_date_show').text(date_d+" "+arr_monthnames[date_m-1]+" "+date_y);
					if(date_m <= 9)
						date_m = '0'+date_m;
					if(date_d <= 9)
						date_d = '0'+date_d;
					$('#appoint_date').val(date_m+"-"+date_d+"-"+date_y);
					//$('#appoint_date').text(date_d+" "+arr_monthnames[date_m-1]+" "+date_y);
                   ...
                   // alert(can_history_api());
                    ...
                    if((is_ie() && version_ie()<=9) || can_history_api()){
						...
						if(_t.hasClass('hasEvent')==false){
							$(o.mode_blogevents_container).html("<h3>No events for today.</h3>");
							$(o.mode_blogevents_container).addClass('active');
						}
						else
						{
			...
			function datepicker_blogevents_click_day(e,arg_t){
